...
class GantScraper extends BaseScraper {
  ...
  protected async scrapeCategory(category: CategoryType): Promise<Product[]> {
    ...
    if (category.categoryId) {
      console.log(
        `using Ajax scraping for ${category.name} - ${category.categoryId}... `,
      );
      let page = 0;
      ...
      while (page <= MAX_PAGES) {
        ...
        try {
          ...
          if (!html) {
            this.logProgress(
              'No HTML returned from AJAX (json.result), stopping',
            );
            break;
          }
          ...
          this.logWarning(
            `Failed to fetch AJAX Gant page, falling back to old method: ${e}`,
          );
  ...
  private parseGantProducts(html: string, category: CategoryType): Product[] {
    ...
    productCards.each((_, el) => {
      ...
      const apiColors = Array.from(elem.find('.colors_wrapper a')).map((link) =>
        ucfirst($(link).attr('title')?.toLocaleLowerCase()),
      );
