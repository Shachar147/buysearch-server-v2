...
export class HollisterScraper extends BaseScraper {
  ...
  private async scrapeHollisterCategory(
    ...
    try {
      ...
      while (hasMoreProducts) {
        ...
        this.logProgress(
          `Found ${productItems.length} products on page ${page + 1}`,
        );
        ...
        this.logProgress(
          `Successfully parsed ${pageProducts} products from page ${page + 1}`,
        );
      ...
      this.logProgress(
        `Total products scraped from ${category.name}: ${allProducts.length}`,
      );
      return allProducts;
    } catch (error) {
  ...
  private parseHollisterProduct(
    ...
    try {
      ...
      const titleElement = productItem.find(
        '[data-testid="catalog-product-card-name"]',
      );
      ...
      // Extract URL
      const urlElement = productItem.find(
        '.catalog-productCard-module__product-content-link',
      );
      const relativeUrl = urlElement.attr('href');
      const url = relativeUrl
        ? `https://www.hollisterco.com${relativeUrl}`
        : '';

      // Extract images
      const images: string[] = [];
      const imageElements = productItem.find(
        '.catalog-productCard-module__productCardImage_1, .catalog-productCard-module__productCardImage_2',
      );
      ...
      if (priceVariant === 'discount') {
        ...
        const originalPriceElement = priceElement.find(
          '[data-variant="original"]',
        );
        const discountPriceElement = priceElement.find(
      ...
      const colors: string[] = [];
      const swatchLabels = productItem.find(
        'label[data-label-visible="false"]',
      );
      ...
      if (!title || !price || !url) {
        console.log(
          '❕ No title, price, or url found for product:',
          productItem,
        );
        return null;
      }
