...
export class NewEraScraper extends BaseScraper {
  ...
  private async scrapeNewEraCategory(
  ...
  private parseNewEraProduct(
    ...
    try {
      ...
      const primaryImage = productItem
        .find('.woo-entry-image-main')
        .attr('src');
      const secondaryImage = productItem
        .find('.woo-entry-image-secondary')
        .attr('src');
      ...
      const delElement = productItem.find(
        '.price del .woocommerce-Price-amount',
      );
      const insElement = productItem.find(
      ...
      if (delElement.length > 0 && insElement.length > 0) {
        ...
        const priceElement = productItem.find(
      ...
      // Extract product ID
      const productId =
        productItem.find('.additional-information').attr('data-id') || '';
      ...
      const extractedColors = extractColorsWithHebrew(
        title + ' ' + (productCategory || '') + ' ' + colorText,
        [],
        this.source,
      );
