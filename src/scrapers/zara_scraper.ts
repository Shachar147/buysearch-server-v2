...
class ZaraScraper extends BaseScraper {
  ...
  private extractColorsAndPricesFromDom(
    $: cheerio.CheerioAPI,
    productCard: cheerio.Cheerio<any>,
  ): { colors: string[]; price: number | null; oldPrice: number | null } {
    // Extract all color bubbles (if multiple colors are shown)
    const colorBubbles = productCard.find(
      '.product-grid-product-info-colors__bubble[aria-label]',
    );
    let colors: string[] = [];
    ...
    const priceElem = productCard
      .find(
        '.price-current__amount .money-amount__main, .price__amount .money-amount__main',
      )
      .first();
    let price: number | null = null;
    if (priceElem && priceElem.text()) {
      const match = priceElem
        .text()
        .replace(',', '')
        .match(/[\d.]+/g);
    ...
    const oldPriceElem = productCard
      .find('.price-old__amount .money-amount__main')
      .first();
    let oldPrice: number | null = null;
    if (oldPriceElem && oldPriceElem.text()) {
      const match = oldPriceElem
        .text()
        .replace(',', '')
        .match(/[\d.]+/g);
  ...
  private parseZaraProduct(
    ...
    const productCard = $(`a[href='${url}']`).closest(
      'li.product-grid-product',
    ) as cheerio.Cheerio<any>;

    // Extract colors and prices from DOM
    const { colors, price, oldPrice } = this.extractColorsAndPricesFromDom(
      $,
      productCard,
    );
    let currency = 'ILS';
    ...
    if (product.offers) {
      fallbackPrice = product.offers.price
        ? Number(product.offers.price)
        : null;
      fallbackOldPrice = product.offers.priceSpecification?.originalPrice
        ? Number(product.offers.priceSpecification?.originalPrice)
        : null;
    ...
    const productJson = {
      ...
      salePercent:
        calcSalePercent(fallbackPrice, oldPrice ?? fallbackOldPrice) ?? 0,
      currency,
      brand,
      categories,
      gender,
  ...
  private async scrapeZaraCategory(category: CategoryType): Promise<Product[]> {
    ...
    while (hasMore && page <= MAX_PAGES) {
      ...
      this.logProgress(
        `extractProductsFromLdJson found: ${products.length} products`,
      );
      if (!products.length) break;
      const pageProducts = products
        .map((p: any) => this.parseZaraProduct(p, category, $))
