...
export class RevolveScraper extends BaseScraper {
  ...
  private async fetchPage(url: string): Promise<string> {
    return fetchPageWithBrowser(url, {
      ...
      onPageReady: async (page) => {
        await page
          .waitForSelector('li.js-plp-container', { timeout: 10000 })
          .catch(() => {
            this.logProgress(
  ...
  private parseProduct(
    ...
    let priceText = '';
    if (isOnSale) {
      priceText = priceContainer
        .find('.price__sale.js-plp-price')
        .text()
        .trim();
      const oldPriceText = priceContainer
        .find('s.price__retail.js-plp-price-retail')
        .text()
        .trim();
      ...
      priceText = priceContainer
        .find('.js-plp-price, .plp_price.price__retail')
        .text()
        .trim();
      if (priceText) {
        const match = priceText.replace(/[$,]/g, '').match(/\d+(\.\d+)?/);
