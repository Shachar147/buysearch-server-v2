...
export class HamashbirScraper extends BaseScraper {
  ...
  private async scrapeHamashbirCategory(category: CategoryType): Promise<Product[]> {
  ...
  private parseHamashbirProduct(productItem: cheerio.Cheerio<any>, category: CategoryType, $: cheerio.CheerioAPI): Product | null {
    try {
      // Extract title
      ...
      if (title.toLowerCase().includes('gift') || title.toLowerCase().includes('גיפט')) {
        return null;
      }
      ...
      const salePriceElement = productItem.find('.price--on-sale .price-item--sale');
      const regularPriceElement = productItem.find('.price--on-sale .price-item--regular');
      ...
      if (salePriceElement.length > 0 && regularPriceElement.length > 0) {
        ...
        const singlePriceElement = productItem.find('.price__regular .price-item--last');
        const singlePriceText = singlePriceElement.text().trim();
        
        if (singlePriceText) {
          const singleMatch = singlePriceText.replace(/[₪,]/g, '').match(/[\d.]+/);
          if (singleMatch) {
      ...
      const productId = productItem.find('img').first().attr('data-item_id') || '';
      ...
      const colors: string[] = [];
      const colorSwatches = productItem.find('.card__variant-list-color .card__variant-list-item');
