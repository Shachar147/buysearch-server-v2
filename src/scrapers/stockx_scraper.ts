...
class StockXScraper extends BaseScraper {
  ...
  private parseStockXProducts(html: string, category: CategoryType): Product[] {
    ...
    productCards.each((_, el) => {
      const elem = $(el);
      const title = elem
        .find('[data-testid="product-tile-title"]')
        .text()
        .trim();
      let url =
        elem
          .find('[data-testid="productTile-ProductSwitcherLink"]')
          .attr('href') || '';
      if (url && !url.startsWith('http')) url = BASE_URL + url;
      // Improved image extraction: skip placeholder images, prefer real product images
      let image = `https://images.stockx.com/images/${title.replace(
        /\s+/g,
        '-',
      )}-Product.jpg?w=150`;
      elem.find('img.chakra-image').each((_, img) => {
        ...
        if (src && !src.includes('Product-Placeholder-Default')) {
          image = src.split('?')[0];
      ...
      const priceStr = elem
        .find('[data-testid="product-tile-lowest-ask-amount"]')
        .text()
        .replace(/[^\d.]/g, '');
      ...
      const colors: string[] = extractColorsWithHebrew(
        title,
        [],
        'stockx_scraper',
      );
  ...
  private async scrapeStockXCategory(
    ...
    while (hasMore && pageNum <= MAX_PAGES) {
      ...
      this.logProgress(
        `Found ${products.length} products in ${category.name} (page ${pageNum})`,
      );
