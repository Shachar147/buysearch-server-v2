...
const CATEGORIES = [
  {
    ...
    gender: 'Men',
    baseUrl: 'men/swimwear',
  },
  {
    id: '175',
    name: Category.JEANS,
    gender: 'Men',
    baseUrl: 'men/pants/jeans',
  },
  {
    id: '169',
    name: Category.T_SHIRTS,
    gender: 'Men',
    baseUrl: 'men/shirts/tshirts',
  },
  {
    id: '171',
    name: Category.SHIRTS,
    gender: 'Men',
    baseUrl: 'men/shirts/dress-shirts',
  },
  {
    id: '367',
    name: Category.BOXERS,
    gender: 'Men',
    baseUrl: 'men/underwear/underpants',
  },
  {
    id: '122',
    name: Category.DRESSES,
    gender: 'Women',
    baseUrl: 'women/dresses',
  },
  {
    id: '128',
    name: Category.JEANS,
    gender: 'Women',
    baseUrl: 'women/pants-skirts/jeans',
  },
  {
    id: '132',
    name: Category.T_SHIRTS,
    gender: 'Women',
    baseUrl: 'women/tops/tshirts',
  },
  ...
  // by brand
  {
    ...
    gender: 'Men',
    baseUrl: 'brands/ralph-lauren',
    additionalFilters: { department_level: { eq: MEN } },
  },
  {
    id: '17345',
    name: 'Polo Ralph Lauren',
    gender: 'Women',
    baseUrl: 'brands/ralph-lauren',
    additionalFilters: { department_level: { eq: WOMEN } },
  },
  {
    id: '17125',
    name: 'Tommy Hilfiger',
    gender: 'Men',
    baseUrl: 'brands/tommy-hilfiger',
    additionalFilters: { department_level: { eq: MEN } },
  },
  {
    id: '17125',
...
class TerminalXScraper extends BaseScraper {
  ...
  private async fetchTerminalXPage(
    categoryId: string,
    page: number,
    pageSize = 24,
    additionalFilters = {},
  ...
  private parseTerminalXProduct(
    ...
    if (item.media_gallery)
      images.push(...item.media_gallery.map((img: any) => img.url));

    const apiColors =
      item.configurable_options
        ?.find((opt: any) => opt.attribute_code === 'color')
        ?.values?.map((v: any) => v.label) || [];
    ...
    const categories = Array.from(
      new Set([
        ...categoryPath,
        ...item.category_ids
          .map((c_id) => categoryIdToName[Number(c_id)])
          .filter(Boolean),
      ]),
    );
  ...
  private async scrapeTerminalXCategory(cat: CategoryType): Promise<Product[]> {
    ...
    do {
      const result = await this.fetchTerminalXPage(
        cat.id as string,
        page,
        undefined,
        cat.additionalFilters,
      );
      const items = result.items || [];
      totalPages = result.page_info?.total_pages || 1;

      this.logProgress(
        `Scanning ${cat.name} page ${page}/${totalPages} (${items.length} items)`,
      );

      allProducts.push(
        ...items.map((item: any) =>
          this.parseTerminalXProduct(
            item,
            [cat.name],
            cat.gender,
            (cat as CategoryType).baseUrl,
          ),
        ),
      );
