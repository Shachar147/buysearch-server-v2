...
const CATEGORIES: CategoryType[] = [
  {
    ...
    gender: 'Men',
    baseUrl:
      'https://www.factory54.co.il/%D7%97%D7%95%D7%9C%D7%A6%D7%AA-%D7%98%D7%99',
  ...
  },
  {
    ...
    gender: 'Men',
    baseUrl:
      'https://www.factory54.co.il/%D7%9E%D7%9B%D7%A0%D7%A1%D7%99-%D7%91%D7%92%D7%93-%D7%99%D7%9D',
  ...
  },
  {
    ...
    gender: 'Men',
    baseUrl:
      'https://www.factory54.co.il/%D7%9E%D7%9B%D7%A0%D7%A1%D7%99-%D7%92%27%D7%99%D7%A0%D7%A1',
  ...
  },
  {
    ...
    gender: 'Men',
    baseUrl:
      'https://www.factory54.co.il/%D7%A0%D7%A2%D7%9C%D7%99-%D7%A1%D7%A0%D7%99%D7%A7%D7%A8%D7%A1',
  ...
  },
  {
    ...
    baseUrl:
      'https://www.factory54.co.il/%D7%97%D7%95%D7%9C%D7%A6%D7%94-%D7%A4%D7%95%D7%9C%D7%95',
  },
  {
    ...
    baseUrl:
      'https://www.factory54.co.il/%D7%A1%D7%95%D7%95%D7%98%D7%A9%D7%99%D7%A8%D7%98',
  },
  {
    ...
    baseUrl:
      'https://www.factory54.co.il/%D7%9E%D7%9B%D7%A0%D7%A1%D7%99%D7%99%D7%9D',
  },
  {
    ...
    baseUrl:
      'https://www.factory54.co.il/%D7%91%D7%95%D7%A7%D7%A1%D7%A8%D7%99%D7%9D',
  },
  {
...
class Factory54Scraper extends BaseScraper {
  ...
  private extractImages($, itemId: string): string[] {
    ...
    if ($pid.length) {
      ...
      $pid.find('[data-hover-image-url]').each((_, el) => {
        ...
        if (hover && !images.includes(hover))
          images.push(hover.startsWith('http') ? hover : BASE_URL + hover);
      });
    }
    return images;
  }

  private extractProductUrl(
    $: cheerio.CheerioAPI,
    itemId: string,
  ): string | null {
  ...
  private parseFactory54Product(
    ...
    const colors = extractColorsWithHebrew(
      title,
      [item.item_variant].filter(Boolean),
      'factory54_scraper',
    );
    ...
    const categories = [
      category.name,
      ...extractCategory(item.item_category),
      ...extractCategory(item.item_category2),
      ...extractCategory(item.item_category3),
      ...extractCategory(item.item_category4),
    ].filter(Boolean);
  ...
  private async scrapeFactory54Category(
    category: CategoryType,
  ): Promise<Product[]> {
    let page = 0;
    ...
    while (hasMore) {
      const url = `${category.url}${
        page > 0 ? `?start=${page * pageSize}` : ''
      }`;
      this.logProgress(`Fetching ${url}`);
      const html = await this.fetchFactory54Page(url);
      const presentProductCount = (
        html.match(/<div class=\"present-product product/g) || []
      ).length;
      this.logProgress(
        `present-product product divs found: ${presentProductCount}`,
      );
      ...
      allProducts.push(
        ...items.map((item: any) =>
          this.parseFactory54Product(item, category, $),
        ),
      );
