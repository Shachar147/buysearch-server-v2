        ...
        for (const selector of acceptSelectors) {
          try {
            ...
            if (button) {
              await button.click();
              this.logProgress(
                `Cookie consent popup handled using selector: ${selector}`,
              );
  ...
  private parseProductsFromHtml(
    ...
    productDivs.each((_, el) => {
      ...
      const brand = normalizeBrandName(
        elem.find('.brand-name').text().trim() || 'Ralph Lauren',
      );
      ...
      elem
        ...
        .each((_, img) => {
          ...
          if (
            ...
            !images.includes(dataSrc) &&
            !dataSrc.includes('swatch')
          )
            images.push(dataSrc);
        });
      ...
      if (price === null) {
        const fallbackPriceText =
          ...
          elem.find('.product-sales-price').text();
    ...
    try {
      ...
      while (hasMoreProducts && pageNum <= MAX_PAGES) {
        ...
        if (pageProducts.length === 0) {
          ...
          this.logProgress(
            `Found ${pageProducts.length} products on page ${pageNum}`,
          );
          ...
          if (pageProducts.length < 20) {
            ...
            this.logProgress(
              `Fewer products than expected on page ${pageNum}, likely reached end`,
            );
            hasMoreProducts = false;
          }
        }
      ...
      this.logProgress(
        `Finished scraping ${category.name}. Total products: ${allProducts.length}`,
      );
    } catch (error) {
      this.logError(
        `Failed to scrape Ralph Lauren category ${category.name}`,
        error,
      );
    }
