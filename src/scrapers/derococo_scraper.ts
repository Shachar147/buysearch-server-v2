...
export class DerococoScraper extends BaseScraper {
    ...
    while (page <= maxPages) {
      ...
      try {
        ...
        for (let i = 0; i < productCards.length; i++) {
          try {
            const product = await this.parseProduct(
              $,
              productCards[i],
              category,
            );
            if (product) {
              ...
              if (this.totalProductsProcessed % 10 === 0) {
                this.logProgress(
                  `Total products processed so far: ${this.totalProductsProcessed}`,
                );
              }
            }
          } catch (error) {
        ...
        if (this.areArraysEqual(currentPageUrls, prevPageUrls)) {
          this.logProgress(
            'Same products as previous page, stopping pagination',
          );
          break;
        }
  ...
  private async fetchPage(url: string): Promise<string> {
    return fetchPageWithBrowser(url, {
      ...
      onPageReady: async (page) => {
        await page
          .waitForSelector('.Grid__Cell .ProductItem', { timeout: 10000 })
          .catch(() => {
            this.logProgress(
  ...
  private parseProduct(
    ...
    if (mainImgSrc) {
      const fullImgUrl = mainImgSrc.startsWith('http')
        ? mainImgSrc
        : `https:${mainImgSrc}`;
    ...
    const altImgSrc = $el
      .find('.ProductItem__Image--alternate')
      .attr('data-src');
    if (altImgSrc && !images.includes(altImgSrc)) {
      const fullAltImgUrl = altImgSrc.startsWith('http')
        ? altImgSrc
        : `https:${altImgSrc}`;
    ...
    if (images.length === 0) {
      ...
      if (fallbackImgSrc) {
        const fullImgUrl = fallbackImgSrc.startsWith('http')
          ? fallbackImgSrc
          : `https:${fallbackImgSrc}`;
        images.push(fullImgUrl);
      }
    }
    ...
    const colors = this.extractDerococoColors(title, url);
    ...
    for (const segment of urlSegments) {
      ...
      if (colorMatch) {
        ...
        if (this.isLikelyColorName(potentialColor)) {
          const formattedColor = potentialColor
            .split(' ')
            .map(
              (word) =>
